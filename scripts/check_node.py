...
async def main():
    ...
    console.print(
        Panel.fit("[bold cyan]BSV Teranode Health Check[/bold cyan]", border_style="cyan")
    )
    console.print()
    ...
    for result in results:
        status_style = (
            "green" if "✓" in result["status"] else "yellow" if "⚠" in result["status"] else "red"
        )
        if "✗" in result["status"]:
            all_ok = False
        table.add_row(
    ...
    if not rpc_user or not rpc_pass:
        ...
        console.print(
            "  Copy .env.example to .env and set TERANODE_RPC_USER and TERANODE_RPC_PASSWORD"
        )
