...
from .storage import DataMetadata, DataType, DatasetStorage
...
class DatasetRetrieval:
    ...
    def __init__(self, client: Optional[BSVClient] = None, config: Optional[Config] = None):
        """
    ...
    def _extract_op_return_data(self, tx_data: dict) -> Optional[bytes]:
        """
    ...
    async def get_chunked(
        ...
        for i, txid in enumerate(chunk_txids):
            ...
            if not result.success:
                return RetrievalResult(
                    success=False,
                    error=f"Failed to retrieve chunk {i + 1}: {result.error}"
                )
    ...
    async def get_by_hash(self, data_hash: str) -> RetrievalResult:
        ...
        return RetrievalResult(
            success=False,
            error="Hash-based lookup not yet implemented. Use txid instead."
        )
